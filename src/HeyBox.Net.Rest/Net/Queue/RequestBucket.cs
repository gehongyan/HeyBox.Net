            ...
            try
            {
                    ...
                    switch (response.StatusCode)
                    {
                        ...
                        default:
                            ...
                            if (response.Stream != null)
                                try
                                {
                                    responseBase = await JsonSerializer.DeserializeAsync<API.Rest.RestResponseBase>(
                                        response.Stream, _serializerOptions);
                                }
                                catch
                                {
                                    // ignored
                            ...
                            throw new HttpException(
                                ...
                                responseBase?.Result is not null
                                    ? [new HeyBoxJsonError("root", [new HeyBoxError(responseBase.Status.RawCode, responseBase.Message)])]
                                    : null
                            );
                    }
                else
                {
                    ...
                    if (response.MediaTypeHeader?.MediaType == MediaTypeNames.Application.Json)
                    {
                        ...
                        if (responseBase?.Status != HeyBoxErrorCode.OK)
                        {
                            throw new HttpException(
                                response.StatusCode,
                                request,
                                responseBase?.Status,
                                responseBase?.Message,
                                responseBase is not null
                                    ? [new HeyBoxJsonError("root", [new HeyBoxError(responseBase.Status.RawCode, responseBase.Message)])]
                                    : null
                            );
                        }

                        return new MemoryStream(Encoding.UTF8.GetBytes(responseBase?.Result.ToString() ?? string.Empty));
                    }
                }
            }
