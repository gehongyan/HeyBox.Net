...
internal class MessageCache
{
    ...
    private readonly ConcurrentQueue<(ulong MsgId, DateTimeOffset Timestamp)> _orderedMessages;
    private readonly int _size;

    public IReadOnlyCollection<SocketMessage> Messages => _messages.ToReadOnlyCollection();

    public MessageCache(HeyBoxSocketClient heyBox)
    {
        _size = heyBox.MessageCacheSize;
        ...
        _orderedMessages = new ConcurrentQueue<(ulong MsgId, DateTimeOffset Timestamp)>();
    }
    ...
    public void Add(SocketMessage message)
    {
        ...
        while (_orderedMessages.Count > _size
               && _orderedMessages.TryDequeue(out (ulong MsgId, DateTimeOffset Timestamp) msg))
    ...
    public SocketMessage? Get(ulong id) => _messages.GetValueOrDefault(id);

    /// <exception cref="ArgumentOutOfRangeException"><paramref name="limit"/> is less than 0.</exception>
    public IReadOnlyCollection<SocketMessage> GetMany(ulong? referenceMessageId,
        Direction dir, int limit = HeyBoxConfig.MaxMessagesPerBatch)
    {
        ...
        return cachedMessageIds
            .Select(x => _messages.GetValueOrDefault(x))
