...
public interface ISocketMessageChannel : IMessageChannel
{
    ...
    ///         <see cref="HeyBox.WebSocket.HeyBoxSocketConfig.MessageCacheSize"/>。
    ///     </note>
    ///     <br />
    ///     此属性从本地的内存缓存中获取消息实体，不会向黑盒语音发送额外的 API 请求。所获取的消息也可能是已经被删除的消息。
    /// </remarks>
    ...
    ///     </note>
    ///     <br />
    ///     此方法从本地的内存缓存中获取消息实体，不会向黑盒语音发送额外的 API 请求。所获取的消息也可能是已经被删除的消息。
    /// </remarks>
    ...
    IReadOnlyCollection<SocketMessage> GetCachedMessages(int limit = HeyBoxConfig.MaxMessagesPerBatch);
    ...
    IReadOnlyCollection<SocketMessage> GetCachedMessages(ulong referenceMessageId, Direction dir, int limit = HeyBoxConfig.MaxMessagesPerBatch);
    ...
    /// </remarks>
    /// <returns> 获取到的多条缓存消息。 </returns>
    IReadOnlyCollection<SocketMessage> GetCachedMessages(IMessage referenceMessage, Direction dir, int limit = HeyBoxConfig.MaxMessagesPerBatch);
}
